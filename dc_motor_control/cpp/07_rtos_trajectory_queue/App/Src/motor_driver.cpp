...
void MotorDriver::ApplySignedDuty(const float signed_duty_permille){
    if (signed_duty_permille > 0.01){
        this->SetCommand(MotorDirection::Forward, std::abs(signed_duty_permille));
    }else if (signed_duty_permille < -0.01) {
        this->SetCommand(MotorDirection::Reverse, std::abs(signed_duty_permille));
    }else{
        this->Stop();
    }
