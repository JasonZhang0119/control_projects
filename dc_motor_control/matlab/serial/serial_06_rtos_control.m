...
% STM32 UART 输出格式：
%
%   dt_ms,speed_ref_x1000,speed_meas_x1000,position_ref_x1000,position_meas_x1000,duty_cycle_x1000
...
fprintf("Expected format: dt_ms,speed_ref,speed_meas,position_ref,position_meas,duty_cycle\n");
...
if enable_live_plot
    ...
    ax2 = nexttile;
    position_ref_line = animatedline(ax2, "LineWidth", 1.2, "Color", "r");
    position_meas_line = animatedline(ax2, "LineWidth", 1.2, "Color", "b", "LineStyle", "--");
    grid(ax2, "on");
    xlabel(ax2, "Time [s]");
    ylabel(ax2, "Position [rad]");
    title(ax2, "Position reference and measurement");
    legend(ax2, "position ref", "position meas");
...
plot(data_table.time_s, data_table.speed_error_rad_s, "b", "LineWidth", 1.2);
grid on;
xlabel("Time [s]");
